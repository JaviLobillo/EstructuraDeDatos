...
public:
    ...
    int count_interval(const T& lower, const T& upper) const {
        return empty() ? 0 : int_count_interval(raiz, lower, upper);
    }

    // FIN METODO EJERCICIO 87

protected:

    // METODO EJERCICIO 87

    int int_count_interval(Link raiz, const T& lower, const T& upper) const {
        if (raiz == nullptr)
            return 0;
        int n;
        if (menor(raiz->elem, lower))
            n = int_count_interval(raiz->dr, lower, upper);
        else if (menor(upper, raiz->elem))
            n = int_count_interval(raiz->iz, lower, upper);
        else
            n = 1 + int_count_interval(raiz->iz, lower, upper) + int_count_interval(raiz->dr, lower, upper);
        return n;
    }
